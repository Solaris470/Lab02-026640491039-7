...
export default function HerbDetail(){
    ...
    useEffect(() => {
        try {
            const fetchPetData = async () => {
                const data = await fetch(`http://localhost:3004/api/getPetById/${petId}`);
                if (data.ok) {
                    const petJson = await data.json();
                    setPetData(petJson);
                    console.log(petJson);
                } else {
    ...
    return (
    <div className="m-3">
        <a href='/lab02/petList'>[ ข้อมูลสัตว์เลี้ยง ]</a>
        <h1 className="font-bold">รายละเอียดสัตว์เลี้ยง</h1>
        {
            <div key={petData.petId}>
